...
class ParquetTarget(BaseDaskTarget):
    """ This is a Luigi Dask Parquet Target.
    When the flag parameter is set, this supports atomic writing using a flag indicator file.
    The target provides reading and writing methods for CSV file sets."""
    def _read(self, path, **kwargs):
        """ This implements the reading of Parquet files.
        Args:
            path: path: str, directory of the data files
            **kwargs: dictionary, named parameters passed to dask.dataframe.read_parquet().
        Returns:
            dask dataframe
        """
...
class CSVTarget(BaseDaskTarget):
    """ This is a Luigi Dask CSV Target.
    The to_csv() method behaves differently compared to to_parquet().
    The implementation of CSVTarget tries to provide better consistency by:
        1. creating parent directores for CSV targets
        2. provide a default glob "part.*.csv" to mimic the behavior of to_parquet().
    When the flag parameter is set, this supports atomic writing using a flag indicator file.
    The target provides reading and writing methods for CSV file sets."""

    def __init__(self, path, glob="part.*.csv", flag=FLAG, storage_options=None):
        """ Constructor for CSVTarget.
        This overrides the BaseDaskTarget constructor to provide a default for glob.
        The default is chosen such that it aligns with the ParquetTarget writing behavior.
        This supports reading and writing a set of files, one per partition within a directory.
        Args:
            path: str, directory of the data files
            glob: str, pattern to support multi partition writing, to align with
                the pattern of ParquetTarget writing. default: "part.*.csv"
            flag: str, when set, implements atomic writing by convention, default: '_SUCCESS'
        Returns:
            instance of CSVTarget
        """
        super().__init__(path, glob, flag, storage_options)

    def _read(self, path, **kwargs):
        """ This implements the reading of CSV files.
        Args:
            path: path: str, directory of the data files
            **kwargs: dictionary, named parameters passed to dask.dataframe.read_csv().
        Returns:
            dask dataframe
        """
